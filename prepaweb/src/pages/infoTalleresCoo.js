...
import { cantidad, campusEx } from "./DashboardC";

console.log("estoy en tabla ->", cantidad)

function InfoTalleresCoo() {
    ...
    function getInscripciones() {
        ...
        var q = query(collection(db, "Inscripcion"), where("campus", "==", campusEx));
        console.log(campusEx)
        getDocs(q)
            .then(response => {
                console.log(response.docs)
    ...
    function filtrar (value) {
        ...
        var q;
        if(value == "Todos"){
            q = query(collection(db, "Inscripcion"), where("estatus", "in", ["Cursando", "Aprobado"], where("campus", "==", campusEx)));
        } else{
            q = query(collection(db, "Inscripcion"), where("estatus", "==", value), where("campus", "==", campusEx));
        }
    ...
    function filtrare (value) {
        ...
        var q;
        if(value == "Todos"){
            q = query(collection(db, "Inscripcion"), where("campus", "==", campusEx));
        } else{
            q = query(collection(db, "Inscripcion"), where("campus", "==", value), where("campus", "==", campusEx));
        }
            ...
            <div className="container bg-white rounded p-5" id="tab">
                <div className="row my-3">
                    <div className="col bold">
                        <h2><i className="fa-solid fa-list-check fa-2x"></i><strong>Informacion de inscripciones - Campus {campusEx}</strong></h2>
                    </div>
                </div>
                <div className="row my-3">
                </div>
...
export default InfoTalleresCoo;